...
- Done: `+page.svelte` split into modular components under `src/lib/components/` (`Window`, `Onboarding`, `Loading`, `Triage`, `Sorting`, `Focus`, `Done`) — the page itself is now just the phase state machine + API calls, composed with `<Window>` per phase.
